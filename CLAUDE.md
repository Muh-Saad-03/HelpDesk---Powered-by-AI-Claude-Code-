...
- **Use the shared enums in `core/`, not magic strings**: every Prisma enum (currently `Role`, future `TicketStatus`, `TicketCategory`, etc.) is mirrored as a `const`-object + type in `core/src/enums/<name>.ts` and imported as `import { Role } from "core"` from both client and server. Compare with `u.role === Role.ADMIN`, never `u.role === "ADMIN"` — string literals scatter, can't be renamed safely, and lose IDE autocomplete. The mirrors are structurally compatible with Prisma's generated enums, so server code can pass `Role.AGENT` directly into Prisma `data` args. When you add a new enum to `schema.prisma`, also add it under `core/src/enums/` and re-export from `core/src/index.ts`.
